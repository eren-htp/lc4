...
import type { PeintureInterieurProduct } from "@shared/peintureInterieurProducts";
import {
  Home,
  Phone,
  Mail,
...
export default function PeintureInterieurProducts() {
  ...
  const marques = Array.from(
    new Set(PEINTURE_INTERIEUR_PRODUCTS.map((p) => p.brand))
  );

  const filteredProducts = PEINTURE_INTERIEUR_PRODUCTS.filter((product) => {
    const matchUsage =
      usageFilter === "Tous" || product.usage.includes(usageFilter);
    const matchMarque =
      marqueFilter === "Tous" || product.brand === marqueFilter;
    return matchUsage && matchMarque;
  });

  const hasActiveFilters = usageFilter !== "Tous" || marqueFilter !== "Tous";
          ...
          <div className="relative z-10 text-center px-4">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-2 mb-6">
              ...
              <span className="text-sm font-medium">
                {PEINTURE_INTERIEUR_PRODUCTS.length} produits disponibles
              </span>
            </div>
            ...
            <p className="text-lg md:text-xl max-w-3xl mx-auto text-white/80">
              Laques et peintures de finition pour vos murs, plafonds et boiseries intérieures
            </p>
          </div>
        </section>
...
function ProductCard({
  ...
  product: PeintureInterieurProduct;
  index: number;
}) {
  return (
      ...
      <div className="group bg-white rounded-2xl border border-gray-200 overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-[#0D1B3E]/30 hover:-translate-y-1 h-full flex flex-col">
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="flex gap-2 mt-auto">
            <Link
              href={`/peintures/peinture-interieur/produit/${product.id}`}
              className="flex-1"
            >
