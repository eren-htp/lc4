...
import { BOIS_PRODUCTS } from "@shared/boisProducts";
import type { BoisProduct } from "@shared/boisProducts";
import {
  Home,
  Phone,
  Mail,
...
type SousCategorieFilter = "Tous" | "lasure" | "solvante" | "acrylique";

const SOUS_CATEGORIE_LABELS: Record<string, string> = {
  lasure: "Lasure",
  solvante: "Laque Boiserie — Solvanté",
  acrylique: "Laque Boiserie — Acrylique",
};

export default function BoisProducts() {
  const searchString = useSearch();
  const urlParams = new URLSearchParams(searchString);
  const marqueParam = urlParams.get("marque");
  const sousCategParam = urlParams.get("type");

  const [usageFilter, setUsageFilter] = useState<UsageFilter>("Tous");
  const [marqueFilter, setMarqueFilter] = useState<string>(marqueParam || "Tous");
  const [sousCategorieFilter, setSousCategorieFilter] = useState<SousCategorieFilter>(
    (sousCategParam as SousCategorieFilter) || "Tous"
  );

  // Extraire les marques uniques
  const marques = Array.from(new Set(BOIS_PRODUCTS.map((p) => p.marque)));

  // Filtrage
  const filteredProducts = BOIS_PRODUCTS.filter((product) => {
    const matchUsage = usageFilter === "Tous" || product.usage.includes(usageFilter);
    const matchMarque = marqueFilter === "Tous" || product.marque === marqueFilter;
    const matchSousCateg = sousCategorieFilter === "Tous" || product.sous_categorie === sousCategorieFilter;
    return matchUsage && matchMarque && matchSousCateg;
  });

  const hasActiveFilters = usageFilter !== "Tous" || marqueFilter !== "Tous" || sousCategorieFilter !== "Tous";

  const resetFilters = () => {
    setUsageFilter("Tous");
    setMarqueFilter("Tous");
    setSousCategorieFilter("Tous");
  };
        ...
        <section className="bg-gray-50 border-b border-gray-200">
          <div className="container mx-auto px-4 py-3">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              <span>/</span>
              <Link href="/peintures/bois" className="hover:text-[#0D1B3E] transition-colors">
                Bois
              </Link>
              <span>/</span>
          ...
          <div className="relative z-10 text-center px-4">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-2 mb-6">
              ...
              <span className="text-sm font-medium">
                {BOIS_PRODUCTS.length} produits disponibles
              </span>
            </div>
            ...
            <p className="text-lg md:text-xl max-w-3xl mx-auto text-white/80">
              Lasures, peintures et laques pour la protection et la mise en valeur du bois
            </p>
          </div>
        </section>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
                  <button
                    onClick={() => setSousCategorieFilter("Tous")}
                    className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                      sousCategorieFilter === "Tous"
                  ...
                  {(["lasure", "solvante", "acrylique"] as SousCategorieFilter[]).map(
                    (option) => (
                      <button
                        key={option}
                        onClick={() => setSousCategorieFilter(option)}
                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                          sousCategorieFilter === option
                            ? "bg-[#0D1B3E] text-white shadow-sm"
                            : "text-gray-600 hover:text-[#0D1B3E] hover:bg-gray-200"
                        }`}
                      >
                        {SOUS_CATEGORIE_LABELS[option]}
                      </button>
                    )
                  )}
                </div>
...
function ProductCard({ product, index }: { product: BoisProduct; index: number }) {
  return (
      ...
      <div className="group bg-white rounded-2xl border border-gray-200 overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-[#0D1B3E]/30 hover:-translate-y-1 h-full flex flex-col">
        ...
        <div className="relative bg-gradient-to-b from-gray-50 to-gray-100 p-6 flex items-center justify-center h-64 overflow-hidden">
          ...
          <div className="absolute top-3 right-3">
            <span className="inline-block bg-[#0D1B3E]/90 backdrop-blur-sm text-white px-3 py-1 rounded-full text-xs font-bold shadow-sm">
              {SOUS_CATEGORIE_LABELS[product.sous_categorie]}
            </span>
          </div>
        </div>
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          {product.couleurs && product.couleurs.length > 0 && (
            <p className="text-xs text-gray-500 mb-3">
              <span className="font-semibold">{product.couleurs.length} teintes</span> disponibles
            </p>
          )}
          ...
          <div className="flex gap-2 mt-auto">
            <Link href={`/peintures/bois/produit/${product.id}`} className="flex-1">
              <Button className="w-full bg-[#0D1B3E] hover:bg-[#0D1B3E]/90 text-sm">
                Fiche détail
              </Button>
            </Link>
            <a href={product.url_produit} target="_blank" rel="noopener noreferrer">
              <Button
                variant="outline"
                size="icon"
