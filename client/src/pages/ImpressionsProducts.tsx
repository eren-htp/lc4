...
import { IMPRESSIONS_PRODUCTS } from "@shared/impressionsProducts";
import type { ImpressionProduct } from "@shared/impressionsProducts";
import {
  Home,
  Phone,
  Mail,
...
type MarqueFilter = "Toutes" | string;

export default function ImpressionsProducts() {
  const searchString = useSearch();
  const urlParams = new URLSearchParams(searchString);
  const marqueParam = urlParams.get("marque");

  const [usageFilter, setUsageFilter] = useState<UsageFilter>("Tous");
  const [marqueFilter, setMarqueFilter] = useState<MarqueFilter>("Toutes");

  // Pré-filtrer par marque si le paramètre est dans l'URL
  useEffect(() => {
    if (marqueParam) {
      // Trouver la marque correspondante dans les produits
      const matchingMarque = IMPRESSIONS_PRODUCTS.find(
        (p) => p.marque.toLowerCase() === marqueParam.toLowerCase()
      );
      if (matchingMarque) {
        setMarqueFilter(matchingMarque.marque);
      }
    }
  }, [marqueParam]);
  ...
  const marques = Array.from(
    new Set(IMPRESSIONS_PRODUCTS.map((p) => p.marque))
  );

  // Filtrage
  const filteredProducts = IMPRESSIONS_PRODUCTS.filter((product) => {
    const matchUsage =
      usageFilter === "Tous" || product.usage.includes(usageFilter);
    const matchMarque =
      marqueFilter === "Toutes" || product.marque === marqueFilter;
    return matchUsage && matchMarque;
  });

  const hasActiveFilters =
    usageFilter !== "Tous" || marqueFilter !== "Toutes";

  const resetFilters = () => {
    setUsageFilter("Tous");
    setMarqueFilter("Toutes");
  };
      ...
      <main className="flex-1">
          ...
          <div className="relative z-10 text-center px-4">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-2 mb-6">
              ...
              <span className="text-sm font-medium">
                {IMPRESSIONS_PRODUCTS.length} produits disponibles
              </span>
            </div>
            ...
            <p className="text-lg md:text-xl max-w-3xl mx-auto text-white/80">
              Produits d'impression, apprêts et sous-couches pour travaux
          ...
          <div className="container mx-auto px-4 py-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
                  <button
                    onClick={() => setMarqueFilter("Toutes")}
                    className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                      marqueFilter === "Toutes"
                        ? "bg-[#0D1B3E] text-white shadow-sm"
                        : "text-gray-600 hover:text-[#0D1B3E] hover:bg-gray-200"
        ...
        <section className="bg-white pt-8 pb-2">
          <div className="container mx-auto px-4">
            <Link href="/peintures/impressions">
              <Button
                variant="outline"
        ...
        <section className="bg-[#0D1B3E] text-white py-16">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-lg mb-8 max-w-2xl mx-auto text-white/80">
              ...
              choix de vos impressions et apprêts.
            </p>
      ...
      <div className="group bg-white rounded-2xl border border-gray-200 overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-[#0D1B3E]/30 hover:-translate-y-1 h-full flex flex-col">
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="flex gap-2 mt-auto">
            <Link
              href={`/peintures/impressions/produit/${product.id}`}
              className="flex-1"
            >
