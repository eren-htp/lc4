...
import type { MetalProduct } from "@shared/metalProducts";
import {
  Home,
  Phone,
  Mail,
...
export default function MetalProducts() {
  ...
  const marques = Array.from(
    new Set(METAL_PRODUCTS.map((p) => p.marque))
  );

  // Filtrage
  const filteredProducts = METAL_PRODUCTS.filter((product) => {
    const matchUsage =
      usageFilter === "Tous" || product.usage.includes(usageFilter);
    const matchMarque =
      marqueFilter === "Tous" || product.marque === marqueFilter;
          ...
          <div className="relative z-10 text-center px-4">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-2 mb-6">
              ...
              <span className="text-sm font-medium">
                {METAL_PRODUCTS.length} produits disponibles
              </span>
            </div>
            ...
            <p className="text-lg md:text-xl max-w-3xl mx-auto text-white/80">
              Peintures et finitions pour menuiseries, structures métalliques et éléments en métal
            </p>
          </div>
        </section>
      ...
      <div className="group bg-white rounded-2xl border border-gray-200 overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-[#0D1B3E]/30 hover:-translate-y-1 h-full flex flex-col">
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="flex gap-2 mt-auto">
            <Link
              href={`/peintures/metal/produit/${product.id}`}
              className="flex-1"
            >
